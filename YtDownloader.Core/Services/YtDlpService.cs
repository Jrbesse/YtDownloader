...
/// <summary>
/// Wraps the yt-dlp executable as a managed subprocess, parsing its stdout for
/// real-time progress updates. ffmpeg is invoked automatically by yt-dlp
/// for post-processing (muxing, audio extraction, etc.).
/// </summary>
public class YtDlpService
{
    public static string YtDlpPath         => ToolPath("yt-dlp");
    public static string FfmpegPath        => ToolPath("ffmpeg");
    public static string FfprobePath       => ToolPath("ffprobe");
    public static string AtomicParsleyPath => ToolPath("AtomicParsley");

    private static string ToolPath(string name) =>
        Path.Combine(PlatformPaths.Current.ToolsDir, name + PlatformPaths.Current.ExecutableExtension);

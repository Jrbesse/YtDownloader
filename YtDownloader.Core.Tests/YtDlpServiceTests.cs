...
public class YtDlpServiceTests
{
    ...
    {
        var absoluteTemplate = OperatingSystem.IsWindows()
            ? @"C:\Videos\%(title)s.%(ext)s"
            : "/Videos/%(title)s.%(ext)s";

        var options = BasicOptions();
        options.CustomOutputTemplate = absoluteTemplate;
        var args = Args(options);
        var oIndex = args.IndexOf("-o");
        args[oIndex + 1].Should().Be(absoluteTemplate);
    }
    ...
    public void BuildArguments_AlwaysContainsFfmpegLocationFlag()
    {
        ...
        var expectedName = "ffmpeg" + PlatformPaths.Current.ExecutableExtension;
        args[ffmpegIndex + 1].Should().EndWith(expectedName);
    }
    ...
    // ── Tool paths (IPlatformPaths) ──────────────────────────────────────────────

    [Fact]
    public void ToolPaths_AreDerivedFromPlatformPathsToolsDir()
    {
        var original = PlatformPaths.Current;
        try
        {
            PlatformPaths.Current = new FakePlatformPaths
            {
                ToolsDir            = Path.Combine("fake", "tools"),
                ExecutableExtension = ".xyz",
            };

            YtDlpService.YtDlpPath.Should().Be(Path.Combine("fake", "tools", "yt-dlp.xyz"));
            YtDlpService.FfmpegPath.Should().Be(Path.Combine("fake", "tools", "ffmpeg.xyz"));
            YtDlpService.FfprobePath.Should().Be(Path.Combine("fake", "tools", "ffprobe.xyz"));
            YtDlpService.AtomicParsleyPath.Should().Be(Path.Combine("fake", "tools", "AtomicParsley.xyz"));
        }
        finally
        {
            PlatformPaths.Current = original;
        }
    }

    [Fact]
    public void ToolPaths_OnNonWindowsPlatform_HaveNoExtension()
    {
        var original = PlatformPaths.Current;
        try
        {
            ...
            YtDlpService.YtDlpPath.Should().Be(Path.Combine("fake", "tools", "yt-dlp"));
        }
        finally
        {
            PlatformPaths.Current = original;
